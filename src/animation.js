const generateAnimationDrawer = () => {
  // Global letiables available anywhere in this file without passing them.
  let v = 0; // used to modify the vertical position of the rocket and fire.
  let h = 0;
  let lazer = 0;
  let lazer2;
  let lazer3;
  let lazer4;
  let sunH = 0; // used to modify the horizontal position of the sun.
  let sunV = 0;
  let sunHb;
  let sunVb;
  // used to modify the vertical position of the sun.
  let cloudsH1 = 0; // used to modify the horizontal position of the left clouds.
  let cloudsH2 = 0; // used to modify the horizontal position of the right clouds.
  let green = 255; // used to modify the green color aspect of the sky.
  let blue = 255; // used to modify the blue color aspect of the sky.
  let launchArmH = 0; // use to modify the horizontal position of the launch arm.
  let launchArmV = 0;
  let count = 10; // contains the value that will be displayed on the clock.
  let LLightR = 0;
  let LLightG = 191;
  let LLightB = 225;
  let RLightR = 255;
  ...
  const paintScene1 = (g, frames) => {
    ...
    // Mountains
    g.setColor(msand);
    let xm = [0, 50, 100, 150, 200, 750, 830, 850, 900, 950, 1000, 1000];
    let ym = [650, 250, 450, 340, 640, 640, 260, 440, 350, 640, 650, 650];
  ...
  const pauseAllSound = () => {
    [launch, lazerFire, explosion, SWTheme].forEach((e) => e.pause());
  };

  const drawAnimation = (g, frames) => {
  ...
  return { drawAnimation, pauseAllSound };
};
